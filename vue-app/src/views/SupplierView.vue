<template>
  ...
  <el-table-column prop="supplier_phone" label="描述" width="200" />
  <el-table-column prop="supplier_address" label="描述" width="200" />
  ...
  <AddSupplier ref="addCategory"
  :dialogTitle="dialogTitle"
  :tableRow="tableRow"></AddSupplier>
    </el-card>
  ...
  //获取分类的信息
  const getList=()=>{
  return axios.get('/File_Management/supplier').then((res)=>{
  ...
  ElMessageBox.confirm(
    ...
    .then(() => {
      axios.delete(`/File_Management/deleteSupplier_Table/${id}`).then(()=>{
        ElMessage({
        type: 'success',
        message: '删除成功',
      });
